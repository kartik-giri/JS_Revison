...
// const promisedObj = promisedTimeout(5000);
// promisedObj.then(()=>{
//   console.log("Excuted after 5 sec")
// })
...
const callBackHellFunc = (greeting, ms)=>{
  return new Promise((resolve, reject)=>{
    return setTimeout( ()=>{
      resolve(greeting);
    }, ms)
...
const callbackObj = callBackHellFunc("hi", 1000);
callBackHellFunc("hi", 1000).then((greeting)=>{
  console.log(greeting);
  return callBackHellFunc("hello", 3000)
}).then((greeting)=>{
  console.log(greeting)
})